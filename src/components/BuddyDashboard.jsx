...
const BuddyDashboard = () => {
  ...
  const fetchMessages = useCallback(async (bookingId) => {
    try {
      const token = user?.token;
      const response = await fetch(`http://localhost:7777/buddy/messages?bookingId=${bookingId}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchProfile = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        if (data.user.buddyProfile) {
          setProfileData(data.user.buddyProfile);
          setAvailabilityDates(data.user.buddyProfile.availableDates || []);
        }
      } else {
  ...
  const updateBookingStatus = async (bookingId, status) => {
    try {
      const token = user?.token;
      const response = await fetch(`http://localhost:7777/buddy/booking/${bookingId}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
                ...
                {bookings.map(booking => (
                  <div key={booking._id} className="bg-white rounded-lg shadow-md p-6">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="text-lg font-medium">Booking with {booking.customer?.name}</h3>
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  ...
                  <div className="bg-green-50 p-4 rounded-lg">
                    ...
                    <p className="text-xl font-bold">{new Date(earnings.lastPaymentDate).toLocaleDateString()}</p>
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="mt-1">{profileData.baseRate || 'Not set'}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="mt-1">{profileData.expertise || 'Not set'}</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="mt-1">{profileData.languages || 'Not set'}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Bio</label>
                      <p className="mt-1">{profileData.bio || 'Not set'}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Available Dates</label>
                      <div className="mt-1">
                        {profileData.availableDates && profileData.availableDates.length > 0 ? (
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <form onSubmit={updateAvailability}>
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700">Add Available Date</label>
