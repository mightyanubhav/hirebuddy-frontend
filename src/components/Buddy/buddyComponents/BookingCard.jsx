    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-md hover:shadow-lg transition-all duration-300 w-full max-w-sm mx-auto sm:mx-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between px-4 py-2 border-b border-gray-200 bg-gray-50 rounded-t-2xl gap-2 sm:gap-0">
        ...
        <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
          {(booking.status === "Confirmed" || booking.status === "Declined" || booking.status === "Pending") && (
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 m-3">
        <div className="grid grid-cols-1 gap-2 text-sm">
          <div className="flex justify-between items-center text-gray-600">
            <div className="flex items-center gap-1"><FontAwesomeIcon icon={faCalendar} className="text-blue-500 w-3" /> Date</div>
            <span className="font-medium text-gray-900">{new Date(booking.date).toLocaleDateString()}</span>
          </div>
          <div className="flex justify-between items-center text-gray-600">
            <div className="flex items-center gap-1"><FontAwesomeIcon icon={faClock} className="text-purple-500 w-3" /> Time</div>
            <span className="font-medium text-gray-900">{booking.time || "Flexible"}</span>
          </div>
          <div className="flex justify-between items-center text-gray-600">
            <div className="flex items-center gap-1"><FontAwesomeIcon icon={faMapMarkerAlt} className="text-red-500 w-3" /> Location</div>
            <span className="font-medium text-gray-900 truncate max-w-[120px]">{booking.location}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 m-3">
        ...
        {booking.serviceDescription && <p className="text-gray-500 text-xs mt-1">{booking.serviceDescription}</p>}
      </div>
      ...
      {(booking.status === "Pending" || booking.status === "Confirmed") && (
        <div className="flex gap-2 p-3 m-3 flex-wrap">
          {booking.status === "Pending" && (
            <>
              <button onClick={() => updateBookingStatus("Confirmed")} className="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded-md text-sm flex items-center justify-center gap-1">
                <FontAwesomeIcon icon={faCheck} /> Accept
              </button>
              <button onClick={() => updateBookingStatus("Declined")} className="flex-1 bg-red-600 hover:bg-red-700 text-white py-2 rounded-md text-sm flex items-center justify-center gap-1">
                <FontAwesomeIcon icon={faTimes} /> Decline
              </button>
            </>
          )}
          {booking.status === "Confirmed" && (
            <button onClick={() => updateBookingStatus("Completed")} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-md text-sm flex items-center justify-center gap-1">
